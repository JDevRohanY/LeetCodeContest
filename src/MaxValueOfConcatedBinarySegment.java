...
public class MaxValueOfConcatedBinarySegment {
    static void main() {
        int[] nums1 = {3,1};
        int[] nums0 = {0,3};
        System.out.println(maxValue(nums1,nums0));
    }
    public static int maxValue(int[] nums1, int[] nums0) {
        ...
        PriorityQueue<Pair> pairs = new PriorityQueue<>(
                (a, b) -> {
                    if (a.ones != b.ones) return b.ones - a.ones;
                    return b.zeros - a.zeros;
                }
        );

        for (int i = 0; i < n; i++) {
            pairs.add(new Pair(nums1[i], nums0[i]));
        }

        StringBuilder ans = new StringBuilder();
        while (!pairs.isEmpty()) {
            Pair top = pairs.poll();          // Bug 2 fix: poll not peek
            for (int i = 0; i < top.ones; i++) ans.append("1");
            for (int i = 0; i < top.zeros; i++) ans.append("0");
        }

        return convertToBinary(ans.toString());
    }

    public static int convertToBinary(String number){
